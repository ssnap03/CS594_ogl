...
bool loadOBJ(
	...
	std::vector<unsigned int> vertexIndices, uvIndices, normalIndices, vertexIndices2, uvIndices2, normalIndices2;
	std::vector<glm::vec3> temp_vertices; 
	std::vector<glm::vec2> temp_uvs;
	std::vector<glm::vec3> temp_normals;
	...
	while( 1 )
	{
		...
		if( strcmp( lineHeader, "o" ) == 0 ){
			count++;
			if(count > 2){ printf("Only two objects can be read at max!\n"); break;}
	...
	for( unsigned int i=0; i<vertexIndices.size(); i++ ){
		...
		// Put the attributes in buffers
		out_vertices1.push_back(vertex);
		out_uvs1     .push_back(uv);
		out_normals1 .push_back(normal);
	...
	if(count==2)
	{
		...
		for( unsigned int i=0; i<vertexIndices2.size(); i++ ){
			...
			unsigned int uvIndex = uvIndices2[i];
			unsigned int normalIndex = normalIndices2[i];
			
			// Get the attributes thanks to the index
			glm::vec3 vertex = temp_vertices2[ vertexIndex-1 - v_count];
			glm::vec2 uv = temp_uvs2[ uvIndex-1 - vt_count];
			glm::vec3 normal = temp_normals2[ normalIndex-1 - vn_count];
			
			// Put the attributes in buffers
			out_vertices2.push_back(vertex);
			out_uvs2     .push_back(uv);
			out_normals2 .push_back(normal);
